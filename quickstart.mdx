...
<Steps>
  ...
  <Step title="Run composer setup">
    ...
    2. **`php artisan key:generate`** — writes `APP_KEY` into your `.env`
    3. **`php artisan migrate --force`** — creates all database tables (SQLite file is created if missing)
    4. **`npm install`** — installs all Node packages including Vite, Tailwind, and Alpine.js
    5. **`npm run build`** — compiles and fingerprints all frontend assets into `public/build/`
