---
title: "ORVIAN Module System and Feature Flags"
...
**Modules not yet in `config/modules.php`** (e.g., `notas`, `horarios`) are commented out. Uncomment the entry to expose it in the module navbar once the implementation is ready.
...
The school's plan is eager-loaded when needed and the `hasFeature()` check is delegated to the plan:
...
// app/Models/Tenant/School.php (line 162)
// $school->hasFeature('attendance_qr')
// → delegates to $this->plan->hasFeature($featureSlug)
```
...
<Tip>
  ...
  <x-ui.app-tile
      ...
      title="Asistencia"
      subtitle="Control"
      :active="$school->hasFeature('attendance_qr')"
      url="{{ route('app.attendance.dashboard') }}" />
