...
|---|---|
| `StudentService` | CRUD for student records, RNC validation, section assignment |
| `StudentPhotoService` | Profile photo capture, storage, and cleanup |
...
| Service | Responsibility |
|---|---|
| `PlantelAttendanceService` | Gate-level attendance recording, tardiness calculation against school shift times, session validation |
| `ClassroomAttendanceService` | Subject-level roll-call, cross-domain consistency enforcement, pasilleo detection |
| `ExcuseService` | Justification workflow — submission, review, approval, retroactive attendance correction |
| `DailySessionManager` | Opening and closing `DailyAttendanceSession` records; prevents attendance recording on days without an open session |
...
| Service | Responsibility |
|---|---|
| `SchoolService` | School creation, status management, plan assignment |
...
After the school and director are created, an event is dispatched:

```php
// app/Events/Tenant/SchoolConfigured.php
event(new SchoolConfigured($school, $director));
```

`SchoolConfigured` carries the school and director models. Laravel's event discovery automatically registers all listeners in `app/Listeners/` without manual registration in `EventServiceProvider`.

### Listeners

Three listeners respond to `SchoolConfigured`, executing asynchronously via the database queue:
...
| `AssignInitialRoles` | Confirms the `School Principal` role assignment on the Director user within the tenant scope |
