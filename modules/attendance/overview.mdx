...
|----------------|-------------------|-------|
| **Absent** (`absent`) | `ClassroomAttendanceRecord` is blocked — teacher cannot mark the student present. The system auto-applies `absent` status. | None (expected behaviour) |
...
| **Excused** (`excused`) | Status propagates automatically to all `ClassroomAttendanceRecord` entries for that day. Teacher cannot override it to present. | None |
