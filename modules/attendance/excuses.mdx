...
The Excuse module provides a structured workflow for absence justifications submitted on behalf of students. When a coordinator approves an excuse, `ExcuseService` immediately applies it retroactively: every `PlantelAttendanceRecord` and `ClassroomAttendanceRecord` that falls within the excuse's date range for that student is updated to `excused` status. This retroactive update ensures that already-closed sessions and teacher roll calls reflect the correct justified status without requiring manual corrections across multiple records.

<Note>
  The `AttendanceExcuseObserver` listens for state transitions on the `AttendanceExcuse` model and fires domain events accordingly. This is the integration point used by notification listeners (e.g. WhatsApp alert suppression) and any future webhook subscribers.
</Note>
...
When `ExcuseService::approveExcuse()` is called (triggered from `ExcuseIndex::approve()`), the `AttendanceExcuseObserver` reacts to the model's `updated` event and initiates retroactive marking:
...
Each matching record is updated to `status = 'excused'`.

**Classroom records:** The same date-range query is applied to `ClassroomAttendanceRecord` — all per-subject records for the student within the excuse window are updated to `status = 'excused'`.
